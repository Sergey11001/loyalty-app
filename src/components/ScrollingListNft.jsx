...
const ScrollingListNft = ({nftListItem, twoRows=false, delisting=false}) => {
    ...
    useEffect(() => {
        if(!scrolledCount && !!refScrollingItem && !!refScrollingBody.current){
            refScrollingBody.current.style.left = -refScrollingItem?.current?.offsetWidth * scrolledCount +"px"
        }
        else if (refScrollingItem && !!refScrollingItem && !!refScrollingBody.current) {
            refScrollingBody.current.style.left = -(refScrollingItem?.current?.offsetWidth + parseInt(window.getComputedStyle(refScrollingBody.current).gap)) * scrolledCount  +"px"
        }
    },[scrolledCount])
    ...
    useEffect(() => {
        ...
        if (!!refScrollingItem && !!refScrollingBody.current) {
                        ...
                        nftListItem.length > 0 ? (
                            ...
                            <div className={twoRows ?  "collection--body__scrolling collection--body__scrolling_short" : "collection--body__scrolling"} ref={refScrollingBody}>
                            {
                                nftListItem.map((item,i)=>(
                                    <div className="collection--item" ref={i===0 ? refScrollingItem : null} key={item.id}>
                                        <div className="collection--item__image">
                                            <img src={item.uri} alt="" className="collection--img"/>
                                        </div>
